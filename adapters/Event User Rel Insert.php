...
$endpointBase = getenv('ENDPOINT_BASE') ?: '';
fwrite(STDERR, "🔗 Final endpointBase: $endpointBase\n");
// === Helpers ===
...
foreach ($lines as $rowIndex => $line) {
    ...
    $eventId = normalizeEmpty($row["event"] ?? "");
    $userId = normalizeEmpty($row["user"] ?? "");

    if ($eventId === "" || $userId === "") {
        fwrite(STDERR, "⚠️ Skipping row missing Event or User: " . json_encode($row) . "\n");
        $skipped++;
        continue;
    }
